...
def gen_seller_chart(tmpl, city):
    ...
    object_list = ReDeal.nonexpires.values('pk', 'title', 'bought', 'thumbnail', 'deal_url', 'website__name').filter(division=city).order_by('-bought')[:settings.SELLER_CHART]
    if city.slug:
        label = '销售排行榜'
...
def gen_view_chart(tmpl, city):
    ...
    for pk in pre_deals:
        try:
            object_list.append(ReDeal.objects.values('pk', 'title', 'thumbnail', 'deal_url', 'website__name').filter(pk=pk)[0])
        except:
            pass
    if city.slug:
        label = '点击排行榜'
